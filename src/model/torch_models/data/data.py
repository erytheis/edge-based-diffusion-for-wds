...
class GraphData(torch_geometric.data.Data):
    ...
    @classmethod
    def from_list(cls, data_list: List[torch_geometric.data.Data]):
        assert all_equal([d.keys for d in data_list])

        kwargs = {}
        for key in data_list[0].keys:
            attr = getattr(data_list[0], key)
            if isinstance(attr, torch.Tensor):
                if key == 'edge_index':
                    cat_dim = 1
                else:
                    cat_dim = 0
                kwargs[key] = torch.cat([getattr(data_list[i], key) for i in range(len(data_list))], dim=cat_dim)
            elif isinstance(attr, str):
                kwargs[key] = [getattr(data_list[i], key) for i in range(len(data_list))]
